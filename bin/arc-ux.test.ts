...
test("thread-merge writes merged_into on existing subscription rows", async () => {
  ...
  expect(r.exitCode).toBe(0);
  const got = rows<{ external_ref: string; merged_into: string | null }>(
    "SELECT external_ref, merged_into FROM thread_subscriptions WHERE thread_id='t-src' ORDER BY external_ref",
  );
  expect(got).toEqual([
    { external_ref: "alice", merged_into: "t-dest" },
    { external_ref: "bob", merged_into: "t-dest" },
  ]);
});
...
test("thread-merge inserts marker row when no subscriptions exist", async () => {
  ...
  expect(r.exitCode).toBe(0);
  const got = rows<{ thread_id: string; module: string; external_ref: string; merged_into: string | null }>(
    "SELECT thread_id, module, external_ref, merged_into FROM thread_subscriptions WHERE thread_id='empty-src'",
  );
  expect(got).toEqual([
    {
      thread_id: "empty-src",
      module: "__merge_marker__",
      external_ref: "__merge_marker__",
      merged_into: "empty-dest",
    },
  ]);
});
...
test("thread_subscriptions schema has merged_into column and index", async () => {
