...
export function collectWorktreeFacts(
  ...
  if (upstream.rc === 0) {
    ...
    const def = run(["-C", entry.path, "symbolic-ref", "--short", "refs/remotes/origin/HEAD"]);
    const ref = def.rc === 0 ? def.stdout.trim().replace(/^origin\//, "") : "main";
    ...
    unpushedBasis = "(detached — no branch to count)";
  }

  return { entry, dirtyFiles, dirtyTop, unpushedCommits, unpushedBasis, lastCommitAgeDays };
}
...
export function findingBody(
  ...
  lines.push(
    `- unpushed commits: ${facts.unpushedCommits} (basis: ${facts.unpushedBasis})`,
    `- linked row: ${linked.rowId ? `${linked.rowId} (${linked.state})` : "none"}`,
    `- classification: ${verdict.action} — ${verdict.reason}`,
  );
  const cmd = suggestedCommand(verdict.action, e.path, e.prunable);
