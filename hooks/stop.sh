...
{
  "decision": "block",
  "reason": "AFK shutdown checklist — task $ARC_TASK_ID is still in state '$STATE'.\n\nBefore exiting, route ONE of these through the bookie subagent:\n  • Done?  → update --state merged --evidence \"<one-line>\" --pr <url-or-branch>\n  • Unrecoverable? → update --state failed --evidence \"<one-line>\"\n  • Needs a human? → decompose <task> --child \"<HITL step>\" [--child ...]  (cap 5)\n\nHygiene phase (suggested, not enforced): once the primary task is terminal/blocked, ALSO route any slice-out-of-scope observations to bookie as new ready hygiene rows. Per G-0005 (one slice per worktree per commit), don't bundle cleanup into the primary slice — emit followups instead:\n  • clarify-docs           — doc/code drift, glossary fixes, broken refs\n  • improve-architecture   — refactor for clarity / dedup (no behavior change)\n  • trash-retired-files    — move unused files to .trash/ with 30d TTL\n  • analyse-recent-sessions — distill recurring friction from recent worker logs\nemit via: bookie create --kind task --class hygiene --title \"<skill>: <observation>\" --body \"<target files + observation>\"\n\nAlso suggested (not enforced): check that docs in scope are still accurate; commit as the configured git user; remove your worktree with 'git worktree remove'.\n\nOnce the ledger is in a terminal or blocked state, this hook will pass through automatically."
}
EOF
exit 0
