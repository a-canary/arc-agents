...
if [ "$TOOL" = "Bash" ]; then
  if echo "$PAYLOAD" | grep -qE 'rm\s+-(rf|fr|f)'; then
    echo "HOOK_BLOCKED: rm -rf is blocked. Move target into ~/trash/\$(date +%s)_<name>-\$(date +%Y%m%d)/ instead (reversible, 30-day retention). A bundled 'to-trash' helper is planned but not yet ported into arc-agents (see ADR 0004)."
    exit 1
  fi
