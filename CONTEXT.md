...
## Default Runner (merge-truth)
The `defaultRunner(project)` factory in `src/ledger/merge-truth.ts` creates a `Runner` that spawns `git`/`gh` processes pinned to the project repo's cwd. Three behaviors distinguish it from the pre-fix runner (analysis-1783937189 Pattern 1): (1) cwd is resolved from the `project` field via `resolveProjectRepo()` instead of inheriting the caller's directory; (2) stderr is merged into stdout so failure messages carry the actual reason (e.g. "fatal: not a git repository") instead of a trailing colon; (3) a 30s wall-clock timeout kills the child and exits 124 if the process hangs — merge-base is in-memory; a corrupt `.git` could hang the validator forever (`DEFAULT_RUNNER_TIMEOUT_MS = 30_000`). See `src/ledger/merge-truth.ts:164-203`.
