...
4. slice-guard — G-0005 PR-scope: diff ≤ `SLICE_GUARD_MAX_LINES` (default 2000 modified-line equivalents) AND touches ≤ `SLICE_GUARD_MAX_AREAS` (default 1) top-level path segments. Non-bypassable: catches PRs that accumulated past the per-commit hook.

   **Paired-area exception**: a single thin-vertical may legitimately touch two areas when the slice is an endpoint + its companion static asset (e.g. `bin/` + `assets/` for a HITL panel). Use `SLICE_GUARD_SKIP=1 git commit ...` — this bypass is the documented mechanism for that exception. Do not broaden the exception to cover unrelated multi-area changes.
