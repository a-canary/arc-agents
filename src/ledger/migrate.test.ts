...
test("014 creates artifacts table with FK to issues", () => {
  const db = fresh();
  const cols = db
...
test("014 creates thread_subscriptions with state CHECK", () => {
  const db = fresh();
  db.run(
...
test("014 renames hitl_deliveries -> deliveries with target_kind backfill", () => {
  const db = new Database(":memory:");
  migrateUpTo(db, "013_event_kind_reclaimed");
  db.run(
    `INSERT INTO hitl_prompts (id, kind, class, payload, recommended, timeout_sec)
...
test("014 deliveries CHECK enforces target_kind enum", () => {
  const db = fresh();
  expect(() =>
    db.run(
